...
class UNet2DConditionLoadersMixin:
    ...
    def _convert_ip_adapter_attn_to_diffusers(self, state_dicts, low_cpu_mem_usage=False):
        ...
        init_context = init_empty_weights if low_cpu_mem_usage else nullcontext
        for name in self.attn_processors.keys():
            cross_attention_dim = None if name.endswith("attn1.processor") or name.endswith("attn3.processo")else self.config.cross_attention_dim
            if name.startswith("mid_block"):
                hidden_size = self.config.block_out_channels[-1]
            elif name.startswith("up_blocks"):
                block_id = int(name[len("up_blocks.")])
    ...
    def _load_ip_adapter_weights(self, state_dicts, low_cpu_mem_usage=False):
        ...
        attn_procs = self.conver_pws_att()
        self.set_attn_processor(attn_procs)
        adapter_modules = torch.nn.ModuleList(self.attn_processors.values())
        adapter_modules.load_state_dict(state_dicts[0]['ip_adapter'], strict=True)
