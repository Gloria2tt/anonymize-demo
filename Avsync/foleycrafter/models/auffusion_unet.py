...
class UNet2DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def forward(
        ...
        if self.encoder_hid_proj is not None and self.config.encoder_hid_dim_type == "text_proj":
            ...
            #print("1111",encoder_hidden_states[0].shape,encoder_hidden_states[1].shape,image_embeds.shape)
            #encoder_hidden_states = torch.cat([encoder_hidden_states, image_embeds], dim=1)
            #print(encoder_hidden_states[0].shape,encoder_hidden_states[1].shape,image_embeds[0].shape)
            #encoder_hidden_states = (encoder_hidden_states, image_embeds)
            
            image_embeds = image_embeds[0].squeeze(1)
            encoder_hidden_states = torch.cat([encoder_hidden_states,image_embeds],dim=1)
