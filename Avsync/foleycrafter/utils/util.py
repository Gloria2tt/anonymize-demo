...
def load_ckpt_to_stylenet(model, ckpt_path):
    # 加载checkpoint
...
def build_foleycrafter(
    ...
    unet = af_UNet2DConditionModel.from_pretrained(pretrained_model_name_or_path, subfolder="unet",low_cpu_mem_usage=False,device_map=None)

    unet_1 = UNet2DConditionModel.from_pretrained(pretrained_model_name_or_path, subfolder="unet",low_cpu_mem_usage=False,device_map=None)
    ...
    controlnet = ControlNetModel.from_unet(unet_1, conditioning_channels=1)
    ipa = IPAdapter.from_pretrained(
                                    audio_base_pretrain=pretrained_model_name_or_path,
                                    pretrained_path='./cross_1031/checkpoint-200000/controlnet',
                                    ckpt_path='./checkpoints/semantic/semantic_adapter.bin'
                                    )
    #ckpt = './stylenet_1028/checkpoint-40000'
    
    
    #stylenet = SytleNet.from_pretrained(ckpt)
    ...
    return pipe,ipa#,stylenet.audio_proj_model
